...
my $unanalyzed_deck_res = $dbh->selectall_arrayref( "SELECT d.id, e.event_type_id FROM decks d JOIN events_players ep ON( d.id = ep.deck_id ) JOIN events e ON( e.id = ep.event_id ) WHERE d.archetype_id IS NULL ORDER BY e.date DESC", { Slice => {} } );
#my $unanalyzed_deck_res = $dbh->selectall_arrayref(    "SELECT d.id, e.event_type_id FROM decks d JOIN events_players ep ON( d.id = ep.deck_id ) JOIN events e ON( e.id = ep.event_id ) WHERE d.id = 13430 ORDER BY e.date ASC",    { Slice => {} } );
...
foreach my $deck ( @{$unanalyzed_deck_res} ) {
    ...
    my $archetypes_res = $dbh->selectall_arrayref(
        "SELECT ac.* FROM archetypes_cards ac JOIN archetypes a ON( ac.archetype_id = a.id ) WHERE event_type_id = ?",
        { Slice => {} },
        $event_type_id
    );
	...
	for(my $archetype_card_index = 0; $archetype_card_index < scalar(@{$archetypes_res}); $archetype_card_index++){
            ...
            if ( $card->{'card_id'} == @{$archetypes_res}[$archetype_card_index]->{'card_id'} ) {
                print
                    "\t\tfound a card match $card->{'card_id'} in archetype  @{$archetypes_res}[ $archetype_card_index ]->{'archetype_id'}\n";
    ...
    my %counts;
    $counts{$_}++ for @matched_archetypes;

    foreach my $archetype_id ( reverse sort keys %counts ) {
        ...
        for my $archetype_card ( @{$archetypes_res} ) {
            if ( $archetype_card->{'archetype_id'} == $archetype_id ) {
                print "\tfound a missing archetype card for $archetype_id - voiding archetype selection\n";
                $check = undef;
                last;
            }
        }
        ...
        if ( $check ) {
            print "\tdeck id '$deck_id' matches archetype id '$archetype_id' - saving\n";
